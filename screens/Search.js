...
export default class Search extends Component {
  ...
  fetchMoreTransactions = async () => {
    ...
    if (enteredtext[0].toUpperCase() === 'B') {
      const t = await db.collection("transaction").where(bookId, '==', text).startAfter(this.state.lastVisibleTransaction).limit(10).get()
      t.docs.map((doc) => {
        this.setState({
      ...
      const t = await db.collection("transaction").where('studentId', '==', text).startAfter(this.state.lastVisibleTransaction).limit(10).get()
      t.docs.map((doc) => {
        this.setState({
          allTransactions: [...this.state.allTransactions, doc.data()],
          lastVisibleTransaction: doc
    ...
    console.log("fetch :  " + this.state.allTransactions.length)
